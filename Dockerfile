...
# Add short U-Mamba debug trainers for smoke testing
RUN python3 -c "from pathlib import Path; p=Path('/opt/U-Mamba/umamba/nnunetv2/training/nnUNetTrainer/nnUNetTrainerUMambaEnc_short.py'); p.write_text(\"\"\"import torch\nfrom nnunetv2.training.nnUNetTrainer.nnUNetTrainerUMambaEnc import nnUNetTrainerUMambaEnc\n\n\nclass nnUNetTrainerUMambaEnc_1epoch(nnUNetTrainerUMambaEnc):\n    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True, device: torch.device = torch.device('cuda')):\n        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)\n        self.num_epochs = 1\n\n\nclass nnUNetTrainerUMambaEnc_5epochs(nnUNetTrainerUMambaEnc):\n    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True, device: torch.device = torch.device('cuda')):\n        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)\n        self.num_epochs = 5\n\"\"\")"
